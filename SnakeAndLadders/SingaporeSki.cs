...
namespace SnakeAndLadders
{
    class SingaporeSki
    {
        ...
        #region Notused
        /*
        public void BFS(int[] level,bool[] visited,int[] parent,int[] parentLevels,int[] input)
        {
            for (int i = 1; i <= Size*Size; i++)
            {
                      ...
                      if (visited[node])
                        ...
                        else
                        {
                            ...
                            foreach (var list in adjacencyList[node])
                            {
                                ...
                                level[list.Value] = (level[list.Value] == 0)
                                    ? level[node] + 1
                                    : Math.Max(level[list.Value], level[node] + 1);
                                if (level[list.Value] == level[node] + 1)
                                {
                                    if (input[node] > input[parent[list.Value]])
                                        parent[list.Value] = node;
                                }
                                   
                                max = Math.Max(max, level[list.Value]);
                               // parentLevels[node] = Math.Max(parentLevels[node], level[list.Value]);
                            }


                       }
                    }
                    parentLevels[i] = Math.Max(parentLevels[i], max);
                }
            }
            ...
            foreach (var node in nodes)
            {
                ...
                int startnode = input[c];
                int endnode = input[node];
                maxJump = Math.Max(maxJump, startnode - endnode);
            }
